...
DROP POLICY IF EXISTS "Teams simplified access" ON teams CASCADE;
DROP POLICY IF EXISTS "User teams access" ON user_teams CASCADE;
DROP POLICY IF EXISTS "User teams simplified access" ON user_teams CASCADE;
DROP POLICY IF EXISTS "Links access" ON links CASCADE;
DROP POLICY IF EXISTS "Links simplified access" ON links CASCADE;

-- Create optimized non-recursive policy for teams
CREATE POLICY "Teams access v2"
  ON teams
  FOR ALL
  TO authenticated
  USING (
...
CREATE POLICY "User teams access v2"
  ON user_teams
  FOR ALL
  TO authenticated
  USING (
