...
export function Dashboard() {
  ...
  const orgQuickLinks = profile?.organizations?.map(org => ({
    name: org.name,
  ...
  const teamQuickLinks = profile?.teams?.map(team => ({
    name: team.name,
        ...
        <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg p-4">
          <div className="flex">
            <div className="flex-shrink-0">
              <UserCircle className="h-5 w-5 text-indigo-400" aria-hidden="true" />
            </div>
            <div className="ml-3">
